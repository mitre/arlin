...
def get_data(load_embeddings=False, load_clusters=False):
    ...
    if load_clusters: 
        clusters = da_utils.load_data(file_path='/nfs/lslab2/arlin/data_zoo/LunarLander-v2/clusters/ppo-50000/test_clusters.pkl')
    else:
        ...
        da_utils.save_data(data=clusters,
                        file_path='/nfs/lslab2/arlin/data_zoo/LunarLander-v2/clusters/ppo-50000/test_clusters.pkl')
...
def graph_latent_analytics(embeddings, clusters, dataset):
    ...
    base_path = './outputs/latent_analytics/'
    ...
    analytics_graphing.graph_multiple_data('Latent Analytics', [db_data, conf_data, cluster_data, ep_prog_data], './outputs/combined_graphs/latent_analytics.png')
...
def graph_cluster_analytics(dataset, clusters):
    ...
    base_path = './outputs/cluster_analytics/'
    for data in [[cluster_conf, 'cluster_confidence.png'], [cluster_rewards, 'cluster_rewards.png']]:
        path = os.path.join(base_path, data[1])
        analytics_graphing.graph_individual_data(data[0], path)
        
    analytics_graphing.graph_multiple_data('Cluster Analytics', [cluster_rewards, cluster_conf], './outputs/combined_graphs/cluster_analytics.png')
